	...
	if(($user_id == "")||($user_email == "")) {			//So there is a case where a user_id is set
		...
		if($has_been_confirmed != true) {
			...
			for($cnt = 0; $cnt < count($notification_ids); $cnt++) {
				...
				$unregister_link = "register.php?userid=" . $user_id . "&id=" . urlencode($raw_notification_ids[$cnt]) . "&devicetype=" . $device_types[$cnt] . "&action=remove";
				$unregister_fully_link = "register.php?userid=" . $user_id . "&id=" . urlencode($raw_notification_ids[$cnt]) . "&devicetype=" . $device_types[$cnt] . "&action=removeall";
